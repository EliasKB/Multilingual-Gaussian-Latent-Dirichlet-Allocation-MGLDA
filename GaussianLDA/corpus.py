# -*- coding: utf-8 -*-
""" This file is used for pre-processing of both corpus and word-embedding, Some outputs of this file will be used for GLDA  """
...
class Corpus(object):
        ...
        for index_doc, doc in enumerate(temp_docs): 
            ...
            for index, word in enumerate(doc):          
                ...
                if word not in self.word2id:                      # if unseen
                    ...
                    self.id2word[current_id] = word               # id2word is a dictionay with word and id (exact as above)

                    doc_id[index] = current_id 
                else:                                             # if seen
                    doc_id[index] = self.word2id[word]            # doc_id is a array of ids
    ...
    def process_wordvectors_ny(self, modelvw):               
        """ making words2vec_ny of dictioanry format in fastttext case when we do not use pretrained word vector """
        self.words2vec_ny = {}
    ...
    def process_wordvectors(self, filepath = None):        
        ...
        self.words2vec = KeyedVectors.load_word2vec_format(filepath, binary=False) 
        useable_vocab = 0
        unusable_vocab = 0
        ...
        sum_words = 0
        for idx, doc in self.index2doc.items(): 
            self.index2doc[idx] = [word for word in doc if word in self.vocab_ny]
            sum_words += len(self.index2doc[idx])
        print('vocab_old: ',len(self.vocab), '      vocab_ny: ',len(self.vocab_ny), ' total number of words: ', sum_words )
        print('-----------')
        
        self.word_vec_size = self.words2vec.vector_size             
